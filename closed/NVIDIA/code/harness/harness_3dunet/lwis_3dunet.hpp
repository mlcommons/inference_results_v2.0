...
{
  public:
    Timer(const std::string& tag_) : tag(tag_)
    {
    ...
    ~Timer()
    {
        std::cout << "Timer " << tag << " reports " << total.count() / count << " ms per call for "
...
class SyncQueue
{
  public:
    ...
    void acquire(std::deque<T>& values, std::chrono::microseconds duration = 10000us,
                 size_t size = 1, bool limit = false)
    {
        size_t remaining = 0;
...
class Device
{
  ...
  public:
    Device(size_t id, size_t numCopyStreams, size_t numInferStreams, size_t numCompleteThreads,
           bool enableSpinWait, bool enableDeviceScheduleSpin, size_t batchSize, bool dla,
           bool useSameContext)
        : m_Id(id), m_BatchSize(batchSize), m_EnableSpinWait(enableSpinWait),
          m_EnableDeviceScheduleSpin(enableDeviceScheduleSpin), m_UseSameContext(useSameContext),
          m_DLA(dla), m_CopyStreams(numCopyStreams), m_InferStreams(numInferStreams)
    {
  ...
  private:
    ...
    std::map<cudaStream_t, std::tuple<std::shared_ptr<BufferManager>, cudaEvent_t, cudaEvent_t,
                                      cudaEvent_t, ::nvinfer1::IExecutionContext*>>
        m_StreamState;
...
class Server : public mlperf::SystemUnderTest
{
  ...
  private:
    ...
    void IssueBatch(DevicePtr_t device, size_t batchSize,
                    std::deque<mlperf::QuerySample>::iterator begin,
                    std::deque<mlperf::QuerySample>::iterator end, cudaStream_t copyStream);

    void CopyKiTS19Sample(const DevicePtr_t device,
                          const std::shared_ptr<KiTS19BufferManager>& bufMgr,
                          std::deque<mlperf::QuerySample>::iterator iter, const cudaStream_t stream,
