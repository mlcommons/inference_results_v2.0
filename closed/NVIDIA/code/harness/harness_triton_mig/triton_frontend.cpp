...
void Server_SUT::Init(size_t min_sample_size, size_t max_sample_size,
                      size_t buffer_manager_thread_count, const std::string& backend_path)
{
#if TRITON_FRONTEND_TRACE
...
void SUTShim::ServerQueueHandler(std::string mig_uuid, uint64_t max_sq_size, std::chrono::microseconds sq_timeout)
{
    while (!is_finished())
    {
        ...
        while (samples.empty())
        {
            ...
            {
                std::cerr << "Met exception while acquire SQ for " << mig_uuid << " : " << e.what()
                          << std::endl;
                assert(false);
            }
            if (is_finished())
...
void SUTShim::IssueQuery(const std::vector<mlperf::QuerySample>& samples)
{
    ...
    else if (policy == IssuePolicy::Buffered)
    {
        ...
        for(int i = 0; i < samples.size(); i += m_max_server_buffer_size)
        {
            ...
            std::vector<mlperf::QuerySample>::const_iterator slice_end =
                slice_start + m_max_server_buffer_size;
