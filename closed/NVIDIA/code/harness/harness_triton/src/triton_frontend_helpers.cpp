...
void BatchResponseComplete(TRITONSERVER_InferenceResponse* response, const uint32_t flags,
                           void* userp)
{
    /* Pass the response (and various items from the userp field) to the member
     * Completion function
                                  ...
                                  int64_t* actual_memory_type_id)
{
    if(byte_size != 0)
    {
        ...
        auto pool = (*reinterpret_cast<triton_frontend::PinnedMemoryPoolEnsemble*>(
            userp))[preferred_memory_type_id];
        ...
        *actual_memory_type_id =
            pool->getMemType() == TRITONSERVER_MEMORY_GPU ? preferred_memory_type_id : 0;
        *buffer = block->m_Data;
...
void RequestPool::Create(const size_t initial_element_count, TRITONSERVER_Server* server,
                         Triton_Server_SUT* server_sut, const std::string& model_name,
