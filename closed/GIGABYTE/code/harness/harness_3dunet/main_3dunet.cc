...
DEFINE_bool(end_on_device, false,
            "Perform query D2H copy inside QuerySamplesComplete() callback (untimed)");
...
DEFINE_uint32(unet3d_sw_overlap_pct, 50,
              "Percent value showing 3DUNet sliding window overlaps (uniform in all dimension)");
DEFINE_string(unet3d_sw_gaussian_patch_path, "",
              "Path to 3DUNet preconditioned, coalesced, Gaussian Patch in npy format (.npy)");
...
/* check if file exists; cannot use std::filesystem due to Xavier NX */
...
void doInference()
{
    ...
    CHECK_EQ(does_exist(sut_settings.SW_gaussian_patch_path), 1)
        << "Cannot find Gaussian Patch file";
    ...
    CHECK_EQ(sut_settings.EnableBatcherThreadPerDevice, true)
        << "BatcherThreadPerDevice should be on";
    ...
    if(sut_settings.m_NumaConfig.empty())
    {
        ...
        auto oneQsl = std::make_shared<qsl::SampleLibrary3DUNet>(
            "LWIS_SampleLibrary", FLAGS_map_path, splitString(FLAGS_tensor_path, ","),
    ...
    else
    {
        ...
        qsl = std::shared_ptr<qsl::SampleLibrary3DUNetEnsemble>(
            new qsl::SampleLibrary3DUNetEnsemble(qsls));
    }
    ...
    for(auto& device : devices)
    {
        ...
        std::cout << "  PerSampleCudaMemcpy Calls: " << stats.m_PerSampleCudaMemcpyCalls
                  << std::endl;
