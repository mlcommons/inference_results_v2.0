...
void DLRM_Triton_SUT::Init(size_t min_sample_size, size_t max_sample_size,
...
{
    Concurrent_Frontend_SUT::Init(min_sample_size, max_sample_size, buffer_manager_thread_count,
    ...
    while(inferences_sent < num_inferences)
    {
        ...
        for(size_t idx = 0; idx < m_InputTensors.size(); idx++)
        {
            ...
            FAIL_IF_ERR(TRITONSERVER_InferenceRequestAddInput(request_block->m_Data, name.c_str(),
                                                              std::get<1>(m_InputTensors[idx]),
                                                              shape.data(), shape.size()),
                        "re-adding input");
            FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(
                            request_block->m_Data, name.c_str(), input_data,
                            single_sample_size * num_pairs, m_InputMemoryType, 0),
                        "appending input data");

            AppendInputDataForAllGPUs(request_block->m_Data, index, name.c_str(),
                                      single_sample_size * num_pairs, idx);
            AppendInputDataForAllQSLs(request_block->m_Data, index, name.c_str(),
                                      single_sample_size * num_pairs, idx);
            ...
            {
                FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(
                                request_block->m_Data, name.c_str(), input_data, single_sample_size,
                                m_InputMemoryType, 0),
                            "appending input data padding");
                AppendInputDataForAllGPUs(request_block->m_Data, index, name.c_str(),
                                          single_sample_size, idx);
                AppendInputDataForAllQSLs(request_block->m_Data, index, name.c_str(),
                                          single_sample_size, idx);
            }
        }
...
void DLRM_Triton_SUT::BatchCompletion(TRITONSERVER_InferenceResponse* response,
                                      const BatchResponseMetaData* response_metadata)
{
    ...
    {
        loadgen_responses.emplace_back(mlperf::QuerySampleResponse{
            (response_metadata->m_ResponseId)[i], output0_result,
        ...
        const void* buffer_ptr_inc =
            buffer_ptr + cumulativeNumPairs * TRITONSERVER_DataTypeByteSize(datatype);
...
size_t DLRM_Triton_SUT::GetNumQuerySamplesToAcquire(size_t num_pairs, int num_successful_tries,
                                                    int num_empty_tries)
{
...
void DLRM_Triton_SUT::ProcessBatch()
{
    ...
    while(true)
    {
        ...
        {
            req->samples.insert(req->samples.begin(), remainingSamples.begin(),
                                remainingSamples.end());
        ...
        do
        {
            ...
            if(samples[0].index == 0 && samples[0].id == 0)
            {
                endSample = true;
                break;
            }

            size_t num_pairs_acquired = std::accumulate(
                samples.begin(), samples.end(), 0, [&](size_t i, const mlperf::QuerySample& s) {
                    return qsl->GetNumUserItemPairs(s.index) + i;
                });
            ...
            if(req->samples.size() > 0 && num_pairs_acquired == 0)
            {
                ...
                (num_empty_tries <= m_NumEmptyTries &&
                 num_successful_tries <= m_NumSuccessfulAcquires));
       ...
       // Set the Request Provider
        ...
        for(size_t idx = 0; idx < m_InputTensors.size(); idx++)
        {
            ...
            AppendInputDataForAllGPUs(request_block->m_Data, req->samples.front().index,
                                      name.c_str(), single_sample_size * num_pairs, idx);
            ...
            if(num_pairs % 2)
            {
                ...
                FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(
                                ...
                                input_data, single_sample_size, TRITONSERVER_MEMORY_CPU, 0),
                            "appending input data padding");

                AppendInputDataForAllGPUs(request_block->m_Data, req->samples.front().index,
                                          name.c_str(), single_sample_size, idx);
                AppendInputDataForAllQSLs(request_block->m_Data, req->samples.front().index,
                                          name.c_str(), single_sample_size, idx);
            }
        }
...
void DLRM_Triton_SUT::IssueTritonBatchRequest()
{
    while(true)
    {
        ...
        for(size_t idx = 0; idx < m_InputTensors.size(); idx++)
        {
            ...
            if(!req->isContiguous)
            {
                for(const auto& sample : req->samples)
                {
                    ...
                    FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(
                                    request_block->m_Data, std::get<0>(m_InputTensors[idx]).c_str(),
                                    input_data,
                                    single_sample_size * qsl->GetNumUserItemPairs(sample.index),
                                    m_InputMemoryType, 0),
                                "appending input data");

                    AppendInputDataForAllGPUs(
                        request_block->m_Data, sample.index,
            ...
            {
                int8_t* input_data = (int8_t*)qsl->GetSampleAddress(
                    req->samples[0].index, idx); // Get address of the query

                const size_t single_sample_size = qsl->GetSampleSize(idx);

                FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputData(
                                request_block->m_Data, std::get<0>(m_InputTensors[idx]).c_str(),
                                input_data, single_sample_size * req->batchSize, m_InputMemoryType,
                                0),
                            "appending input data");
                AppendInputDataForAllGPUs(request_block->m_Data, req->samples[0].index,
                                          std::get<0>(m_InputTensors[idx]).c_str(),
                                          single_sample_size * req->batchSize, idx);
        ...
        request_block->m_BatchResponseMetadata.m_PaddingSize =
            (req->batchSize % 2) ? m_OutputPaddingSize : 0;
        FAIL_IF_ERR(TRITONSERVER_InferenceRequestSetResponseCallback(
                        request_block->m_Data, m_Allocator, m_BatchedOutputBufferPool.get(),
                        BatchResponseComplete, &request_block->m_BatchResponseMetadata),
...
void DLRM_Triton_SUT::InitIssueThreads()
{
    ...
    if(m_BatchTritonRequests)
    {
        for(int i = 0; i < m_NumIssueThreads; i++)
            m_IssueThreads.emplace_back(
                std::thread(&DLRM_Triton_SUT::IssueTritonBatchRequest, this));
    }
    else
    {
        for(int i = 0; i < m_NumIssueThreads; i++)
...
bool DLRM_Triton_SUT::CheckContiguity(const std::deque<mlperf::QuerySample>& samples, int num_pairs)
{
    ...
    for(size_t i = 0; i < m_InputTensors.size(); i++)
    {
        ...
        int8_t* last_task_start =
            static_cast<int8_t*>(qsl->GetSampleAddress(samples[samples.size() - 1].index, i));
        ...
        if(first_task_start + sample_size !=
           last_task_start + (num_pairs_last_task * single_sample_size))
        {
            return false;
        }
    }
...
void DLRM_Triton_SUT::AppendInputDataForAllGPUs(TRITONSERVER_InferenceRequest* request,
                                                const mlperf::QuerySampleIndex& sample_idx,
                                                ...
                                                size_t input_idx)
{
    if(m_StartFromDevice && m_NumGPUs > 1)
    {
        for(int i = 0; i < m_NumGPUs; i++)
        {
            ...
            int8_t* input_data =
                (int8_t*)qsl->GetSampleAddress(sample_idx, input_idx, 0,
                                               i); // Get address of the query for device

            FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputDataWithHostPolicy(
                            request, input_name, input_data, input_size, m_InputMemoryType, i,
                                                ...
                                                size_t input_idx)
{
    ...
    if(m_SampleLibraries.size() > 1)
    {
        for(size_t qsl_idx = 0; qsl_idx < m_SampleLibraries.size(); ++qsl_idx)
        {
            ...
            FAIL_IF_ERR(TRITONSERVER_InferenceRequestAppendInputDataWithHostPolicy(
                            request, input_name, input_data, input_size, m_InputMemoryType, 0,
