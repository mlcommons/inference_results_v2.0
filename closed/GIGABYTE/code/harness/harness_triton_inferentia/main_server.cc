...
triton_frontend::ServerSUTPtr_t getInitializedServerSUT()
{
    ...
    sut = std::make_shared<triton_frontend::Concurrent_Frontend_SUT>(
        "Concurrent_Triton_Server", FLAGS_model_store_path, FLAGS_model_name, FLAGS_model_version,
        false, false, FLAGS_pinned_input, FLAGS_request_pool_count, FLAGS_num_batchers,
        FLAGS_num_issuers, FLAGS_is_single_stream);
    ...
    sut->Init(min_sample_size, max_sample_size, FLAGS_buffer_manager_thread_count,
              FLAGS_batch_triton_requests, FLAGS_check_contiguity, "",
              FLAGS_python_backend_path);
