...
void Concurrent_Frontend_SUT::Init(size_t min_sample_size, size_t max_sample_size,
                                   ...
                                   const std::string& backend_path)
{
    Triton_Server_SUT::Init(min_sample_size, max_sample_size, buffer_manager_thread_count,
                            batch_triton_requests, check_contiguity, numa_config_str, backend_path);
    ...
    m_OutputBufferPool.reset(new PinnedMemoryPoolEnsemble(
        pool_item_count_per_instance, pool_item_size, gpu_instance_count, numa_config));
    ...
    if(m_BatchTritonRequests)
    {
        ...
        m_BatchedOutputBufferPool.reset(new PinnedMemoryPoolEnsemble(
...
void Concurrent_Frontend_SUT::InitBatcherThreads()
{
    ...
    if(m_BatchTritonRequests)
    {
        for(int i = 0; i < m_NumBatcherThreads; i++)
            m_BatcherThreads.emplace_back(
                std::thread(&Concurrent_Frontend_SUT::ProcessBatch, this));
    }
    else
    {
        for(int i = 0; i < m_NumBatcherThreads; i++)
            m_BatcherThreads.emplace_back(
                std::thread(&Concurrent_Frontend_SUT::ProcessSample, this));
    }
}
    ...
    if(m_BatchTritonRequests)
    {
        for(int i = 0; i < m_NumIssueThreads; i++)
            m_IssueThreads.emplace_back(
                std::thread(&Concurrent_Frontend_SUT::IssueTritonBatchRequest, this));
    }
    else
    {
        for(int i = 0; i < m_NumIssueThreads; i++)
            m_IssueThreads.emplace_back(
                std::thread(&Concurrent_Frontend_SUT::IssueTritonRequest, this));
    }
}
...
void Concurrent_Frontend_SUT::IssueTritonRequestHelper(mlperf::QuerySampleIndex index,
                                                       mlperf::ResponseId responseID,
                                                       ...
                                                       bool isWarmup)
{
    ...
    for(size_t idx = 0; idx < m_InputTensors.size(); idx++)
    {
        ...
        AppendInputDataForAllGPUs(request_block->m_Data, index,
                                  std::get<0>(m_InputTensors[idx]).c_str(), input_size, idx);
        AppendInputDataForAllQSLs(request_block->m_Data, index,
                                  std::get<0>(m_InputTensors[idx]).c_str(), input_size, idx);
    }
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerInferAsync(m_Server.get(), request_block->m_Data, m_TracePtr),
                "running inference");
...
void Concurrent_Frontend_SUT::Completion(TRITONSERVER_InferenceResponse* response,
...
void Concurrent_Frontend_SUT::BatchCompletion(TRITONSERVER_InferenceResponse* response,
                                              const BatchResponseMetaData* response_metadata)
{
    ...
    for(int i = 0; i < (response_metadata->m_ResponseId).size(); i++)
    {
        loadgen_responses.emplace_back(mlperf::QuerySampleResponse{
            (response_metadata->m_ResponseId)[i], output0_result, batch1_output_size});
    ...
    if(m_ResponseCallback)
    {
        std::vector<::mlperf::QuerySampleIndex> sample_indices =
            response_metadata->m_QuerySampleIdxList;
...
void Concurrent_Frontend_SUT::ReportLatencyResults(
...
bool Concurrent_Frontend_SUT::CheckContiguity(std::deque<mlperf::QuerySample>& samples)
{
    ...
    {
        auto prev =
            static_cast<int8_t*>(m_SampleLibraries[0]->GetSampleAddress(samples.front().index, i));
        ...
        if(req->isContiguous)
        {
            for(size_t idx = 0; idx < m_InputTensors.size(); idx++)
            {
                ...
                AppendInputDataForAllGPUs(request_block->m_Data, req->samples.front().index,
                                          std::get<0>(m_InputTensors[idx]).c_str(), input_size,
                                          idx);
