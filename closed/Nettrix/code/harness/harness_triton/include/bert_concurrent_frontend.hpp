...
{
  public:
    BERT_Triton_SUT(const std::string& name, const std::string& model_repo_path,
                    const std::string& model_name, uint32_t model_version, bool start_from_device,
                    bool pinned_input, uint64_t request_count, size_t num_batcher_threads,
        ...
        : Concurrent_Frontend_SUT(name, model_repo_path, model_name, model_version, false,
                                  ...
                                  num_batcher_threads, num_issue_threads, is_single_stream)
    {
    }
    ~BERT_Triton_SUT() {}
