...
def get_args():
    ...
    parser.add_argument("--merge_query_max_num", default=8, type=int, help="threads")
...
def get_backend(backend, dataset, max_ind_range, data_sub_sample_rate, use_gpu):
    ...
    elif backend == "recaccel":
        from backend_neuchips import BackendRecAccel
        backend = BackendRecAccel()
    else:
...
class Item_id:
    """An item id that we queue for processing by the thread pool."""
...
class Item:
    ...
    def __init__(self, query_id, content_id, ndata, batch_T=None, idx_offsets=None, q_time=None):
        self.query_id = query_id
        self.content_id = content_id
        self.ndata = ndata
...
class RunnerBase:
    ...
    def run_one_item(self, qitem):
        ...
        processed_results = []
        try:
            results = self.model.predict(qitem.ndata, qitem.batch_T.shape[0])
            ...
            response = []
            b0 = processed_results.ctypes.data
            byte_per_result = processed_results[0].nbytes
            for idx, query_id in enumerate(qitem.query_id):
                s_idx = qitem.idx_offsets[idx]
                e_idx = qitem.idx_offsets[idx + 1]
                
                b1 = (e_idx-s_idx)*byte_per_result
                response.append(lg.QuerySampleResponse(query_id, b0, b1)) #0.0015
                b0 += b1
    ...
    def enqueue(self, query_samples):
        ...
        if query_len < self.max_batchsize:
            batch_T, idx_offsets, ndata = self.ds.get_samples(idx)
            self.run_one_item(Item(query_id, idx, ndata, batch_T, idx_offsets))
        else:
            bs = self.max_batchsize
            for i in range(0, query_len, bs):
                ie = min(i + bs, query_len)
                batch_T, idx_offsets, ndata = self.ds.get_samples(idx[i:ie])
                self.run_one_item(Item(query_id[i:ie], idx[i:ie], ndata, batch_T, idx_offsets))
...
class QueueRunner(RunnerBase):
    def __init__(self, model, ds, threads, post_proc=None, max_batchsize=128, merge_query_max_num=1):
        super().__init__(model, ds, threads, post_proc, max_batchsize)
        queue_size_multiplier = 10 #(args.samples_per_query_offline + max_batchsize - 1) // max_batchsize)
        self.merge_query_max_num = merge_query_max_num-1
        assert(self.merge_query_max_num>=0)
    ...
    def handle_tasks(self, tasks_queue):
        ...
        while True:
            ...
            query_id= qitem_id.query_id
            idx = qitem_id.content_id
            q_time = qitem_id.start
            for i in range(self.merge_query_max_num):
                try:
                    qitem_id_t = tasks_queue.get_nowait()
                    query_id.extend(qitem_id_t.query_id)
                    idx.extend(qitem_id_t.content_id)
                    task_num += 1
                except Exception as ex:
                    pass
            batch_T, idx_offsets, ndata = self.ds.get_samples(idx)
            self.run_one_item(Item(query_id, idx, ndata, batch_T, idx_offsets, q_time))
    ...
    def enqueue(self, query_samples):
        ...
        if query_len < self.max_batchsize:
            #batch_T, idx_offsets, ndata = self.ds.get_samples(idx)
            #self.tasks.put(Item(query_id, idx, ndata, batch_T, idx_offsets))
            self.tasks.put(Item_id(query_id, idx))
        else:
            bs = self.max_batchsize
            for i in range(0, query_len, bs):
                ie = min(i + bs, query_len)
                #batch_T, idx_offsets, ndata = self.ds.get_samples(idx[i:ie])
                #self.tasks.put(Item(query_id[i:ie], idx[i:ie], ndata, batch_T, idx_offsets))
                self.tasks.put(Item_id(query_id[i:ie], idx[i:ie]))
...
def main():
    ...
    ds = wanted_dataset(model=backend,
    ...
    runner = runner_map[scenario](model, ds, args.threads, post_proc=post_proc, max_batchsize=args.max_batchsize, merge_query_max_num=args.merge_query_max_num)
