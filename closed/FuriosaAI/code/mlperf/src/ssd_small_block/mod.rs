...
use super::ssd_small::{select_devices, CppPostprocessor};
use crate::utils::{
...
impl SystemUnderTest for FuriosaSUT {
    ...
    fn issue_query(&mut self, samples: &[QuerySample]) {
        ...
        let mut outputs = self.session.create_output_tensors().unwrap();
        self.session.run(&self.inputs, &mut outputs).unwrap();

        let output_buffers = outputs.to_buffers();
        let data = output_buffers.iter().map(|b| b.as_slice::<u8>()).collect::<Vec<_>>();

        complete_a_sample(sample, &data, &self.postprocessor);
    }
...
fn complete_a_sample(query_sample: QuerySample, outputs: &[&[u8]], pp: &CppPostprocessor) {
    let mut result_data = pp.postprocess(query_sample.index as f32, outputs);
