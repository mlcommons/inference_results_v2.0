...
class RN50GraphSurgeon(object):
    ...
    fusion_map = {
        "Res2Mega": ["fuse_res2_mega", "fuse_conv"],
    ...
    def fuse_ops(self):
        ...
        if self.device_type != 'gpu' or self.need_calibration:
            ...
            policy = policy_selector.get(str(self.compute_sm), 'Unknown')
    ...
    def fuse_conv(self):
        logging.info("Fusing conv and add op ")
        op_names_lists_to_be_fused = [["res3d_branch2c", "res3d", "res3d_relu"],
                                      ["res3c_branch2c", "res3c", "res3c_relu"],
                                      ["res3b_branch2c", "res3b", "res3b_relu"]]

        for _idx, op_names_list in enumerate(op_names_lists_to_be_fused):
            # setup plugin info
            plugin_name = "Serial3Conv2d_{}".format(_idx + 1)
            ...
            from_shortcut = op_list[1].i(0, 0).outputs[0]\
                    if op_list[1].i(1, 0).name == op_names_list[0] else\
                    op_list[1].i(1, 0).outputs[0]
            ...
            # NOTE: order does not matter
            plugin_field_dict = {
                "c_br2c_w": op_list[0].inputs[1],
                "s_br2c_b": op_list[0].inputs[2],
                "dynamic_ranges": dyn_const,
            }
            ...
            # replace ops with plugin
            self.graph.RES2PLUGIN("Serial3Conv2d_TRT", plugin_name, plugin_inp, plugin_out, attrs)
