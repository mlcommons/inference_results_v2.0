...
void enqueueShim(nvinfer1::IExecutionContext* context, int batchSize, void** bindings,
                 cudaStream_t stream, cudaEvent_t* inputConsumed)
{
...
void Device::AddEngine(EnginePtr_t engine)
{
    ...
    else
    {
        ...
        batchSize = engine->GetCudaEngine()
                        ->getProfileDimensions(0, 0, nvinfer1::OptProfileSelector::kMAX)
                        .d[0];
    }
...
void Device::Setup()
{
    ...
    for(auto& stream : m_CopyStreams)
    {
        ...
        auto state =
            std::make_tuple(std::make_shared<BufferManager>(aliasPtr, m_BatchSize, profileIdx),
                            cudaEvent_t(), cudaEvent_t(), cudaEvent_t(), context);
        ...
        if(!engine->hasImplicitBatchDimension() && engine->getNbOptimizationProfiles() > 1 &&
           !m_UseSameContext)
        {
            ++profileIdx;
        }
...
void Device::Completion()
{
    ...
    while(true)
    {
        ...
        mlperf::QuerySamplesComplete(&batch.Responses[0], batch.Responses.size(),
                                     batch.ResponseCb.value_or(mlperf::ResponseCallback{}));
...
void Server::Setup(ServerSettings_3DUNet& settings, ServerParams& params)
{
    ...
    for(auto& deviceNum : devices)
    {
        ...
        for(auto& deviceTypes : params.EngineNames)
        {
            for(auto& batches : deviceTypes)
            {
                ...
                for(int32_t deviceInstance = 0;
                    deviceInstance < (isDlaDevice ? runtime->getNbDLACores() : 1); deviceInstance++)
                {
                    if(isDlaDevice && m_ServerSettings.MaxDLAs != -1 &&
                       deviceInstance >= m_ServerSettings.MaxDLAs)
                        continue;

                    size_t numCopyStreams = isDlaDevice ? m_ServerSettings.DLACopyStreams
                                                        : m_ServerSettings.GPUCopyStreams;
                    size_t numInferStreams = isDlaDevice ? m_ServerSettings.DLAInferStreams
                                                         : m_ServerSettings.GPUInferStreams;

                    // this batchSize is  for SW batch size
                    size_t batchSize =
                        isDlaDevice ? m_ServerSettings.DLABatchSize : m_ServerSettings.GPUBatchSize;

                    if(UseNuma())
                    {
                        bindNumaMemPolicy(GetNumaIdx(deviceNum), GetNbNumas());
                    }
                    auto device = std::make_shared<lwis::Device>(
                        deviceNum, numCopyStreams, numInferStreams,
                    ...
                    for(auto& engineName : batches)
                    {
                        ...
                        auto engine =
                            runtime->deserializeCudaEngine(trtModelStream.data(), size, nullptr);
    ...
    CHECK(m_ServerSettings.SW_dhw == 128)
        << "3DUNet KiTS19 only supports 128x128x128 sliding window";
    ...
    for(auto& device : m_Devices)
    {
        ...
        const bool directHostAccess =
            ((device->m_DLA && m_ServerSettings.EnableDLADirectHostAccess) ||
        ...
        m_kits19_buffer_manager[deviceNum] = std::make_shared<KiTS19BufferManager>(
            m_ServerSettings.SW_dhw, device->m_InferStreams.size(),
            sampleLibrary->GetMaxSampleSize(), m_ServerSettings.SW_gaussian_patch_path,
    ...
    // create batchers
    for(size_t deviceNum = 0;
        deviceNum < (m_ServerSettings.EnableBatcherThreadPerDevice ? m_Devices.size() : 1);
        deviceNum++)
    {
    ...
    if(m_ServerSettings.EnableCudaThreadPerDevice)
    {
        for(size_t deviceNum = 0; deviceNum < m_Devices.size(); deviceNum++)
        {
            ...
            if(UseNuma())
            {
                bindThreadToCpus(m_IssueThreads.back(),
                                 GetClosestCpus(m_Devices[deviceNum]->GetId()));
            }
        }
    }
...
void Server::CopyKiTS19Sample(const DevicePtr_t device,
                              const std::shared_ptr<KiTS19BufferManager>& bufMgr,
                              std::deque<mlperf::QuerySample>::iterator iter,
                              const cudaStream_t stream, const bool directHostAccess)
{
    ...
    if(m_ServerSettings.EnableStartFromDeviceMem)
    {
        ...
        CHECK_EQ(cudaMemcpyAsync(static_cast<int8_t*>(bufMgr->getSampleInputDeviceBuffer()),
                                 ...
                                 sampleLibrary->GetSampleSize(iter->index, 0),
    ...
    else if(directHostAccess)
    {
        ...
        std::memcpy(static_cast<int8_t*>(bufMgr->getSampleInputHostBuffer()),
                    sampleLibrary->GetSampleAddress(iter->index, 0),
                    sampleLibrary->GetSampleSize(iter->index, 0));
    ...
    else
    {
        ...
        bufMgr->copyInputToDeviceAsync(stream, sampleLibrary->GetSampleAddress(iter->index, 0),
                                       sampleLibrary->GetSampleSize(iter->index, 0));
        device->m_Stats.m_PerSampleCudaMemcpyCalls++;
    }
...
void Server::IssueBatch(DevicePtr_t device, size_t batchSize,
                        std::deque<mlperf::QuerySample>::iterator begin,
                        std::deque<mlperf::QuerySample>::iterator end, cudaStream_t copyStream)
{
    ...
    auto inferStream = (m_ServerSettings.RunInferOnCopyStreams)
                           ? copyStream
    ...
    for(int d = 0; d < sample_dims.d - stride; d += stride)
    {
        ...
        for(int h = 0; h < sample_dims.h - stride; h += stride)
        {
            ...
            for(int w = 0; w < sample_dims.w - stride; w += stride)
            {
                ...
                sw_buffers[device->m_InferStreamNum][0] =
                    KiTS19bufMgr->getSWInputBuffer(device->m_InferStreamNum);
                sw_buffers[device->m_InferStreamNum][1] =
                    KiTS19bufMgr->getSWOutputBuffer(device->m_InferStreamNum);

                // slicing
                TIMER_START(UNET3DSW_sliceKernel);
                UNet3DKiTS19SliceKernelI8CDHW32_wrapper(sample_in_buf,
                                                        sw_buffers[device->m_InferStreamNum][0],
                                                        &u3p, inferStream, deviceId);
                TIMER_END(UNET3DSW_sliceKernel);

                TIMER_START(enqueueShim);
                enqueueShim(context, enqueueBatchSize, &sw_buffers[device->m_InferStreamNum][0],
                            inferStream, nullptr);
                ...
                UNet3DKiTS19PatchKernel_wrapper(sw_buffers[device->m_InferStreamNum][1],
                                                KiTS19bufMgr->getGaussianPatchBuffer(patch_idx),
                                                sample_out_buf, &u3p, inferStream, deviceId);
                ...
                // managed by hardware.
                device->m_InferStreamNum =
                    (device->m_InferStreamNum + 1) % device->m_InferStreams.size();
            }
        }
    }
    ...
    if(m_ServerSettings.EnableResponse)
    {
        ...
        batch.Responses.emplace_back(
            mlperf::QuerySampleResponse{begin->id, (uintptr_t)buffer, sampleSize});
        if(device->m_ResponseCallback)
            batch.SampleIds.emplace_back(begin->index);
        if(m_ServerSettings.EndOnDevice)
        {
            auto baseDevice =
                reinterpret_cast<uintptr_t>(KiTS19bufMgr->getSampleResponseDeviceBuffer());
            auto baseHost =
                reinterpret_cast<uintptr_t>(KiTS19bufMgr->getSampleResponseHostBuffer());
            batch.ResponseCb = [=](mlperf::QuerySampleResponse* response) {
                auto dbuf = (response->data - baseHost) + baseDevice;
                CHECK_EQ(cudaMemcpyAsync(reinterpret_cast<void*>(response->data),
                                         reinterpret_cast<void*>(dbuf), response->size,
    ...
    if(device->m_Stats.m_BatchSizeHistogram.find(batchSize) ==
       device->m_Stats.m_BatchSizeHistogram.end())
    {
    ...
    while(true)
    {
        ...
        do
        {
            ...
            m_WorkQueue.acquire(samples, m_ServerSettings.Timeout, batchSize,
                                m_ServerSettings.EnableDequeLimit ||
                                    m_ServerSettings.EnableBatcherThreadPerDevice);
...
void Server::Warmup(double duration)
{
    ...
    do
    {
        for(size_t deviceIndex = 0; deviceIndex < m_Devices.size(); ++deviceIndex)
        {
            ...
            for(auto copyStream : device->m_CopyStreams)
            {
                for(auto inferStream : device->m_InferStreams)
                {
                    ...
                    {
                        for(auto i = 0;
                            i < engine->getNbBindings() / engine->getNbOptimizationProfiles(); i++)
                        {
                            if(engine->bindingIsInput(i))
                            {
                    ...
                    {
                        CHECK_EQ(
                            cudaGraphLaunch(g_it->second, (m_ServerSettings.RunInferOnCopyStreams)
                                                              ? copyStream
                                                              : inferStream),
                            CUDA_SUCCESS);
                    }
                    else
                    {
                        ...
                        enqueueShim(context, device->m_BatchSize, &buffers[0],
                                    (m_ServerSettings.RunInferOnCopyStreams) ? copyStream
                                                                             : inferStream,
                                    nullptr);
                    }
                    if(!m_ServerSettings.RunInferOnCopyStreams)
        ...
        elapsed = std::chrono::duration<float>(std::chrono::high_resolution_clock::now() - tStart)
                      .count();
...
void Server::ReportLatencyResults([
    [maybe_unused]] const std::vector<mlperf::QuerySampleLatency>& latencies_ns)
{
}

void Server::SetResponseCallback(
        ...
        callback)
{
    std::for_each(m_Devices.begin(), m_Devices.end(),
                  [callback](DevicePtr_t device) { device->SetResponseCallback(callback); });
}
