    ...
    else
    {
        ...
        for (int32_t numaIdx = 0; numaIdx < nbNumas; numaIdx++)
        {
            ...
            auto constructQsl = [&]()
            {
                ...
                auto oneQsl = std::make_shared<qsl::SampleLibrary>("LWIS_SampleLibrary", FLAGS_map_path, splitString(FLAGS_tensor_path, ","),
                    FLAGS_performance_sample_count ? FLAGS_performance_sample_count : std::max(FLAGS_gpu_batch_size, FLAGS_dla_batch_size),
                    padding, FLAGS_coalesced_tensor, start_from_device);
                resetNumaMemPolicy();
                sut->AddSampleLibrary(oneQsl);
