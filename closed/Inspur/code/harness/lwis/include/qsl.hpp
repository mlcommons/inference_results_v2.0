  ...
  public:
    SampleLibrary(std::string name, std::string mapPath, std::vector<std::string> tensorPaths, size_t perfSampleCount,
      ...
      for (size_t input_idx = 0; input_idx < m_NumInputs; input_idx++) {
        ...
        if (!m_StartFromDevice[input_idx]) {
          ...
          CHECK_EQ(cudaMallocHost(&m_SampleMemory[input_idx][0], (m_PerfSampleCount + m_PerfSamplePadding) * m_SampleSizes[input_idx]), cudaSuccess);
        }
        else {
          ...
          for (int device_idx = 0; device_idx < m_NumDevices; device_idx++) {
            ...
            CHECK_EQ(cudaMalloc(&m_SampleMemory[input_idx][device_idx], (m_PerfSampleCount + m_PerfSamplePadding) * m_SampleSizes[input_idx]), cudaSuccess);
          }
        }
      }
    }
    ...
    void LoadSamplesToRam(const std::vector<mlperf::QuerySampleIndex>& samples) override {
      ...
      if (!m_Coalesced) {
        for (size_t sampleIndex = 0; sampleIndex < samples.size(); sampleIndex++) {
          ...
          for (size_t input_idx = 0; input_idx < m_NumInputs; input_idx++) {
            ...
            npy.loadAll(data);
            if (!m_StartFromDevice[input_idx]) {
              auto sampleAddress = static_cast<int8_t *>(m_SampleMemory[input_idx][0]) + sampleIndex * m_SampleSizes[input_idx];
              memcpy((char *)sampleAddress, data.data(), m_SampleSizes[input_idx]);
            }
            else{
              for (int device_idx = 0; device_idx < m_NumDevices; device_idx++) {
                auto sampleAddress = static_cast<int8_t *>(m_SampleMemory[input_idx][device_idx]) + sampleIndex * m_SampleSizes[input_idx];
                CHECK_EQ(cudaSetDevice(device_idx), cudaSuccess);
                CHECK_EQ(cudaMemcpy(sampleAddress, data.data(), m_SampleSizes[input_idx], cudaMemcpyHostToDevice), cudaSuccess);
              }
            }
          }
        }
        ...
        for (size_t input_idx = 0; input_idx < m_NumInputs; input_idx++) {
          ...
          if (!m_StartFromDevice[input_idx]) {
            ...
            for (int device_idx = 0; device_idx < m_NumDevices; device_idx++) {
              CHECK_EQ(cudaSetDevice(device_idx), cudaSuccess);
              CHECK_EQ(cudaMemcpy(m_SampleMemory[input_idx][device_idx], data.data(), samples.size() * m_SampleSizes[input_idx], cudaMemcpyHostToDevice), cudaSuccess);
            }
          }
        }
      }
      ...
      for (size_t sampleIndex = 0; sampleIndex < samples.size(); sampleIndex++) {
        ...
        for (size_t input_idx = 0; input_idx < m_NumInputs; input_idx++) {
          if (!m_StartFromDevice[input_idx]) {
            m_SampleAddressMapHost[sampleId].back()[input_idx] = static_cast<int8_t *>(m_SampleMemory[input_idx][0]) + sampleIndex * m_SampleSizes[input_idx];
          }
        }
        ...
        for (size_t input_idx = 0; input_idx < m_NumInputs; input_idx++) {
          if (m_StartFromDevice[input_idx]) {
            ...
            for (int device_idx = 0; device_idx < m_NumDevices; device_idx++) {
              m_SampleAddressMapDevice[sampleId].back()[input_idx][device_idx] = static_cast<int8_t *>(m_SampleMemory[input_idx][device_idx]) + sampleIndex * m_SampleSizes[input_idx];
            }
          }
        }
      }
