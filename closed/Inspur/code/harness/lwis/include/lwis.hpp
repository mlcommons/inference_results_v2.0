...
// Reduce lock contention on Orin by making the call
// to SyncQueue::empty() lock-free (MLPINF-1523)
// Ideally we want to port this change to all other
// systems but don't have time to test before 2.0
// Deferring to 2.1 since this touches some delicate
// parts of LWIS and don't want to risk a regression
#ifdef IS_ORIN
#define LOCKFREE_EMPTY 1
#endif
...
namespace lwis {
  ...
  public:
    ...
    {
#if LOCKFREE_EMPTY
        // if the call to empty returns true/false, there
        // there is no guarantee that the queue is empty/not-empty
        // when the call returns
        // this is the same gaurantee that using a lock provides,
        // since the lock is unlocked when the function returns,
        // and could have been modified by another thread in the interim
