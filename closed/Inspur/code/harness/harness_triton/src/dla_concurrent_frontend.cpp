...
void DLA_Triton_SUT::InitDlaIssueThreads()
{
    ...
    if(m_BatchTritonRequests)
    {
        for(int i = 0; i < m_DlaNumIssueThreads; i++)
            m_DlaIssueThreads.emplace_back(
                std::thread(&DLA_Triton_SUT::IssueTritonDlaBatchRequest, this));
    }
    else
    {
        for(int i = 0; i < m_DlaNumIssueThreads; i++)
            m_DlaIssueThreads.emplace_back(
                std::thread(&DLA_Triton_SUT::IssueTritonDlaRequest, this));
    }
}

void DLA_Triton_SUT::Init(size_t min_sample_size, size_t max_sample_size,
                          ...
                          const std::string& backend_path)
{
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerLoadModel(m_Server.get(), m_DlaModelName.c_str()),
                "Failed to load DLA model");
    ...
    while(true)
    {
        ...
        FAIL_IF_ERR(TRITONSERVER_ServerModelIsReady(m_Server.get(), m_DlaModelName.c_str(),
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerModelConfig(m_Server.get(), m_DlaModelName.c_str(),
                                               m_DlaModelVersion, 1 /* config_version */,
    ...
    for(const auto& instance_group : m_Config.instance_group())
    {
        ...
        {
            auto it = dla_instance_count.find(gpu);
            if(it == dla_instance_count.end())
            {
    ...
    if(m_BatchTritonRequests)
    {
        ...
        m_DlaBatchedOutputBufferPool.reset(new PinnedMemoryPoolEnsemble(
            pool_item_count_batch, pool_item_size_batch, dla_instance_count, numa_config));
    ...
    else
    {
        ...
        m_DlaOutputBufferPool.reset(new PinnedMemoryPoolEnsemble(pool_item_count, pool_item_size,
                                                                 dla_instance_count, numa_config));
        LOG(INFO) << "Allocated Pinned memory pool of count: " << pool_item_count
                  << " size :" << pool_item_size << " bytes for DLA requests.";
    }

    DLARequestPool::Create(m_RequestCount /* initial_element_count */, m_Server.get(), this,
                           m_DlaModelName, m_DlaModelVersion, inputs, output_names);
...
void DLA_Triton_SUT::IssueTritonDlaRequest()
{
    while(true)
    {
        ...
        FAIL_IF_ERR(TRITONSERVER_InferenceRequestSetResponseCallback(
                        request_block->m_Data, m_Allocator, m_DlaOutputBufferPool.get(),
...
void DLA_Triton_SUT::ModelStats()
{
    ...
    auto err = TRITONSERVER_ServerModelStatistics(m_Server.get(), m_DlaModelName.c_str(),
                                                  m_DlaModelVersion, &model_stats_message);
    if(err != nullptr)
    {
...
void DLA_Triton_SUT::IssueTritonDlaBatchRequest()
{
    while(true)
    {
        ...
        /* Set response callback for this request */
        FAIL_IF_ERR(TRITONSERVER_InferenceRequestSetResponseCallback(
                        request_block->m_Data, m_Allocator, m_DlaBatchedOutputBufferPool.get(),
