...
DEFINE_bool(is_single_stream, false,
            "If running single stream in concurrent harness, setting this reduces latency");
...
triton_frontend::ServerSUTPtr_t getInitializedServerSUT(bool isDLRM)
{
    ...
    {
        sut = std::make_shared<triton_frontend::BERT_Triton_SUT>(
            "BERT_Triton_Server", FLAGS_model_store_path, FLAGS_model_name, FLAGS_model_version,
            FLAGS_start_from_device, FLAGS_pinned_input, FLAGS_request_pool_count,
    ...
    else
    {
        sut = std::make_shared<triton_frontend::Concurrent_Frontend_SUT>(
            ...
            FLAGS_model_version, FLAGS_use_dlrm_qsl, FLAGS_start_from_device, FLAGS_pinned_input,
            FLAGS_request_pool_count, FLAGS_num_batchers, FLAGS_num_issuers,
            ...
            for(int32_t numaIdx : gpuNumaVec)
            {
                ...
                auto constructQsl = [&]() {
                    ...
                    auto oneQsl = std::make_shared<DLRMSampleLibrary>(
                        ...
                        FLAGS_start_from_device, numaIdx, nbNumas);
                    resetNumaMemPolicy();
