...
size_t Serial3Conv2dPlugin::getSerializationSize() const noexcept
{
...
void Serial3Conv2dPlugin::serialize(void* data) const noexcept  
{
...
void Serial3Conv2dPlugin::setPluginNamespace(const char* libNamespace) noexcept
{
    mNamespace = libNamespace;
}

const char* Serial3Conv2dPlugin::getPluginNamespace() const noexcept
{
    return mNamespace.c_str();
}

int Serial3Conv2dPlugin::initialize() noexcept
{
    ...
    if (!mInitialized)
    {
        ...
        {   
            cudaMalloc(&mDeviceWeights[i], mWeights[i].size() * sizeof(int8_t));
            cudaMemcpy(mDeviceWeights[i], &mWeights[i].front(), mWeights[i].size() * sizeof(int8_t), cudaMemcpyHostToDevice);
        }
...
void Serial3Conv2dPlugin::terminate() noexcept
{
    if (mInitialized)
    {
...
void Serial3Conv2dPlugin::attachToContext(cudnnContext* cudnn, cublasContext* cublas, IGpuAllocator* allocator) noexcept
{
    //std::cout << "Serial3Conv2dPlugin::attachToContext" << std::endl;
}

void Serial3Conv2dPlugin::detachFromContext() noexcept
{
    //std::cout << "Serial3Conv2dPlugin::detachFromContext" << cudaSuccess << std::endl;
}

int Serial3Conv2dPlugin::getNbOutputs() const noexcept
{
...
DimsExprs Serial3Conv2dPlugin::getOutputDimensions (int outputIndex, const DimsExprs *inputs, int nbInputs, IExprBuilder &exprBuilder) noexcept
{
...
void Serial3Conv2dPlugin::destroy() noexcept
{
    //std::cout << "Serial3Conv2dPlugin::destroy" << std::endl;
    delete this;
}

size_t Serial3Conv2dPlugin::getWorkspaceSize(const PluginTensorDesc *inputs, int nbInputs, const PluginTensorDesc *outputs, int nbOutputs) const noexcept
{
...
const char* Serial3Conv2dPlugin::getPluginType() const noexcept
{
    //std::cout << "Serial3Conv2dPlugin::getPluginType" << std::endl;
    return SERIAL3_CONV2D_PLUGIN_NAME;
}

const char* Serial3Conv2dPlugin::getPluginVersion() const noexcept
{
    return SERIAL3_CONV2D_PLUGIN_VERSION;
}

IPluginV2DynamicExt * Serial3Conv2dPlugin::clone() const noexcept
{
...
DataType Serial3Conv2dPlugin::getOutputDataType(int index, const DataType* inputTypes, int nbInputs) const noexcept
{
...
void Serial3Conv2dPlugin::configurePlugin(const DynamicPluginTensorDesc *in, int nbInputs, const DynamicPluginTensorDesc *out, int nbOutputs) noexcept
{
...
const char* Serial3Conv2dPluginCreator::getPluginName() const noexcept
{
    //std::cout << "Serial3Conv2dPluginCreator getPluginName" << std::endl;
    return SERIAL3_CONV2D_PLUGIN_NAME;
}

const char* Serial3Conv2dPluginCreator::getPluginVersion() const noexcept
{
    //std::cout << "Serial3Conv2dPluginCreator getPluginVersion" << std::endl;
    return SERIAL3_CONV2D_PLUGIN_VERSION;
}

const PluginFieldCollection* Serial3Conv2dPluginCreator::getFieldNames() noexcept
{
...
IPluginV2* Serial3Conv2dPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept
{
...
IPluginV2* Serial3Conv2dPluginCreator::deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept
{
...
void Serial3Conv2dPluginCreator::setPluginNamespace(const char* pluginNamespace) noexcept
{
    //std::cout << "Serial3Conv2dPluginCreator::setPluginNamespace" << std::endl;
    mNamespace = pluginNamespace;
}

const char* Serial3Conv2dPluginCreator::getPluginNamespace() const noexcept
{
