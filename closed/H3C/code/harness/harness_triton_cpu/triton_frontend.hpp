...
class Server_SUT : public mlperf::SystemUnderTest
{
  public:
    Server_SUT(std::string name, std::string model_repo_path, std::string model_name,
               uint32_t model_version, std::string openvino_version, bool use_dlrm_qsl, bool start_from_device, bool pinned_input)
        : m_Name(name), m_ModelRepositoryPath(model_repo_path), m_ModelName(model_name),
          m_ModelVersion(model_version), m_OpenvinoVersion(openvino_version), m_UseDlrmQsl(use_dlrm_qsl)
    {
