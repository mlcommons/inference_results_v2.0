                         ...
                         const std::vector<std::string>& outputs)
{
    ...
    {
        instances_.emplace_back(new RequestPool(initial_element_count, server, server_sut,
...
RequestPool::RequestPool(const size_t initial_element_count, TRITONSERVER_Server* server,
                         ...
                         const uint32_t model_version, const std::vector<InputMetaData>& inputs,
                         const std::vector<std::string>& outputs)
    : m_Head(nullptr), m_ReleasedHead(nullptr), m_Blocks(initial_element_count), m_Server(server),
      m_ServerSUT(server_sut), m_ModelName(model_name), m_ModelVersion(model_version),
...
void Server_SUT::Init(size_t min_sample_size, size_t max_sample_size,
                      size_t buffer_manager_thread_count)
{
#if TRITON_FRONTEND_TRACE
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetBackendConfig(server_options, "openvino", "version", m_OpenvinoVersion.c_str()),
                            "setting backend config");
    // Necessary for loading CPU models
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetStrictModelConfig(server_options, false),
                "setting strict model configuration");
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetMinSupportedComputeCapability(server_options, 8.0),
                "setting strict model configuration");

    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetPinnedMemoryPoolByteSize(
                    server_options, ((uint64_t)1) << 29 /* 512 MB */),
                "setting pinned memory pool size");
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetBufferManagerThreadCount(server_options,
    ...
    RequestPool::Create(2500000 /* initial_element_count */, m_Server.get(), this, m_ModelName,
...
void Server_SUT::TraceCaptureTimeStamp(TRITONSERVER_InferenceTrace* trace_ptr,
                                       const std::string comment)
{
    if(m_TraceManager != nullptr)
    {
        if(trace_ptr != nullptr)
        {
            ...
            struct timespec ts;
            clock_gettime(CLOCK_MONOTONIC, &ts);
            m_TraceManager->CaptureTimestamp(trace_id, TRITONSERVER_TRACE_LEVEL_MIN, comment,
                                             TIMESPEC_TO_NANOS(ts));
        }
    }
}
                                   ...
                                   int indexIntoQuerySample, int pool_idx)
{
    ...
    FAIL_IF_ERR(TRITONSERVER_InferenceRequestSetResponseCallback(
                    request_block->m_Data, m_Allocator, m_OutputBufferPool.get(), ResponseComplete,
                    &request_block->m_ResponseMetadata),
                "appending input data");
...
void Server_SUT::HandleSingleBertQuery(const std::vector<mlperf::QuerySample>& samples,
                                       int indexIntoQuerySample, int pool_idx)
{
...
void Server_SUT::HandleSingleDlrmQuery(const std::vector<mlperf::QuerySample>& samples,
                                       int indexIntoQuerySample, int pool_idx)
{
    TRITONSERVER_InferenceTrace* trace = nullptr;
#if TRITON_FRONTEND_TRACE
...
void Server_SUT::IssueQuery(const std::vector<mlperf::QuerySample>& samples)
{
    ...
    {
        auto a1 = std::async(std::launch::async, &Server_SUT::IssueQueryInternal, this, samples, 0,
                             samples.size() / 2);
...
void Server_SUT::IssueQueryInternal(const std::vector<mlperf::QuerySample>& samples,
    ...
    if(isBertBenchmark)
    {
        ...
        for(int samplePos = 0; samplePos < samples.size(); ++samplePos)
        {
            sequenceSamplePosAndLength[samplePos] = std::make_pair(
                samplePos,
                static_cast<int>(GetSampleLength(m_SampleLibrary, samples[samplePos].index)));
        }
