...
def bert_encoder_layer_int8_var_seqlen(cfg, max_seqlen, weights_dict, network,
    ...
    if use_small_tile_gemm_plugin:
        print("use_small_tile_gemm_plugin = true")
        ...
        print("use_small_tile_gemm_plugin = false")
    ...
    pf_skip = trt.PluginField("skipIn", np.array([int(layer >= 20)], np.int32), trt.PluginFieldType.INT32)
    fields = [pf_ld, pf_beta, pf_gamma, pf_type, pf_skip]
    ...
    if use_small_tile_gemm_plugin:
        ...
        if(int(layer)<18):
            ...
            # RELU
            relu_layer = network.add_activation(fc_mid_out, trt.ActivationType.RELU)
            relu_layer.name = prefix + 'relu'
            gelu_out = relu_layer.get_output(0)
