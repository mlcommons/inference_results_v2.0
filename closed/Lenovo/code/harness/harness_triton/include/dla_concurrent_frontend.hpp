...
{
  public:
    DLA_Triton_SUT(const std::string& name, const std::string& model_repo_path,
                   const std::string& model_name, uint32_t model_version,
                   const std::string& dla_model_name, uint32_t dla_model_version,
                   bool start_from_device, bool pinned_input, uint64_t request_count,
                   size_t num_batcher_threads, size_t num_issue_threads, size_t dla_batch_size,
                   size_t dla_num_batchers, size_t dla_num_issuers, float dla_ratio)
        : Concurrent_Frontend_SUT(name, model_repo_path, model_name, model_version, false,
                                  start_from_device, pinned_input, request_count,
                                  num_batcher_threads, num_issue_threads, false),
          m_DlaBatchSize(dla_batch_size), m_DlaModelName(dla_model_name),
          m_DlaModelVersion(dla_model_version), m_DlaNumBatcherThreads(dla_num_batchers),
          m_DlaNumIssueThreads(dla_num_issuers), m_DlaRequestRatio(dla_ratio)
    {
    }
    ~DLA_Triton_SUT() {}

    virtual void Init(size_t min_sample_size = 1, size_t max_sample_size = 1,
