...
public:
        ...
        , mProfileIdx(profileIdx)
    {
        ...
        for (int i = 0; i < numBindings; i++)
        {
            ...
            size_t vol = lwis::volume(mEngine->getBindingDimensions(i), mEngine->getBindingFormat(i), mEngine->hasImplicitBatchDimension());
    ...
    size_t volume(const std::string& tensorName) const
    {
        ...
        if (index == -1)
            return kINVALID_SIZE_VALUE;
        return lwis::volume(mEngine->getBindingDimensions(index), mEngine->getBindingFormat(index), mEngine->hasImplicitBatchDimension());
    }
    ...
    //!
    void copyInputToDeviceAsync(const size_t index, const cudaStream_t& stream = 0, void *src = nullptr, const size_t size = 0, const size_t offset = 0) {
    ...
    //!
    void copyOutputToHostAsync(const size_t index, const cudaStream_t& stream = 0, void *dst = nullptr, const size_t size = 0, const size_t offset = 0) {
...
private:
    ...
    void memcpyBuffers(const size_t index, const bool copyInput, const bool deviceToHost, const bool async, const cudaStream_t& stream = 0,
        void *buf = nullptr, const size_t size = 0, const size_t offset = 0)
    {
        ...
        CHECK((copyInput && mEngine->bindingIsInput(index)) || (!copyInput && !mEngine->bindingIsInput(index)))
            << "Expecting binding " << index << " to be " << (copyInput ? "input" : "output") << "but get the opposite.";

        void* dstPtr = deviceToHost ? (buf ? buf : mManagedBuffers[index]->hostBuffer.data()) : static_cast<char *>(mManagedBuffers[index]->deviceBuffer.data()) + (buf ? offset * size : 0);
        const void* srcPtr = deviceToHost ? static_cast<char *>(mManagedBuffers[index]->deviceBuffer.data()) + (buf ? 0 : offset * size) : (buf ? buf : mManagedBuffers[index]->hostBuffer.data());
