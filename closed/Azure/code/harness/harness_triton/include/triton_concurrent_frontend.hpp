...
{
  public:
    Concurrent_Frontend_SUT(const std::string& name, const std::string& model_repo_path,
                            const std::string& model_name, uint32_t model_version,
                            bool use_dlrm_qsl, bool start_from_device, bool pinned_input,
                            uint64_t request_count, size_t num_batcher_threads,
                            size_t num_issue_threads, bool is_single_stream)
        : Triton_Server_SUT(name, model_repo_path, model_name, model_version, use_dlrm_qsl,
                            start_from_device, false, pinned_input, request_count),
          m_NumBatcherThreads(num_batcher_threads), m_NumIssueThreads(num_issue_threads),
    ...
    virtual void IssueTritonRequestHelper(mlperf::QuerySampleIndex index,
