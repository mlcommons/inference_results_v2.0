                      ...
                      const bool hasImplicitBatch)
{
    ...
    return std::accumulate(d_new.d + (hasImplicitBatch ? 0 : 1), d_new.d + d_new.nbDims, 1,
...
class GenericBuffer
{
  public:
    ...
    //!
    GenericBuffer() : mByteSize(0), mBuffer(nullptr) {}

    //!
    //! \brief Construct a buffer with the specified allocation size in bytes.
    //!
    GenericBuffer(size_t size) : mByteSize(size)
    {
        if(!allocFn(&mBuffer, mByteSize))
            throw std::bad_alloc();
    }

    GenericBuffer(GenericBuffer&& buf) : mByteSize(buf.mByteSize), mBuffer(buf.mBuffer)
    {
        buf.mByteSize = 0;
        buf.mBuffer = nullptr;
    }
...
class BufferManager
{
  public:
    ...
    BufferManager(std::shared_ptr<nvinfer1::ICudaEngine> engine, const int& batchSize,
                  const int profileIdx)
        : mEngine(engine), mBatchSize(batchSize), mProfileIdx(profileIdx)
    {
        ...
        for(int i = 0; i < numBindings; i++)
        {
            ...
            size_t vol =
                lwis::volume(mEngine->getBindingDimensions(i), mEngine->getBindingFormat(i),
    ...
    void dumpBuffer(std::ostream& os, const std::string& tensorName)
    {
        ...
        switch(mEngine->getBindingDataType(index))
        {
            ...
            case nvinfer1::DataType::kBOOL:
                assert(0 && "Bool network-level input and output is not supported");
        }
    }
    ...
    //!
    void copyInputToDeviceAsync(const size_t index, const cudaStream_t& stream = 0,
                                void* src = nullptr, const size_t size = 0, const size_t offset = 0)
    {
    ...
    //!
    void copyOutputToHostAsync(const size_t index, const cudaStream_t& stream = 0,
                               void* dst = nullptr, const size_t size = 0, const size_t offset = 0)
    {
  ...
  private:
    ...
    void memcpyBuffers(const size_t index, const bool copyInput, const bool deviceToHost,
                       const bool async, const cudaStream_t& stream = 0, void* buf = nullptr,
                       const size_t size = 0, const size_t offset = 0)
    {
        ...
        CHECK((copyInput && mEngine->bindingIsInput(index)) ||
              (!copyInput && !mEngine->bindingIsInput(index)))
        ...
        void* dstPtr = deviceToHost
                           ...
                           : static_cast<char*>(mManagedBuffers[index]->deviceBuffer.data()) +
                                 (buf ? offset * size : 0);
...
class KiTS19ManagedBuffer
{
  public:
    ...
    // needs to be zeroed-out for 32ch; one time flushing is enough
...
class KiTS19BufferManager
{
  public:
    ...
    KiTS19BufferManager(const int SW_dhw = 128, const int numInferStream = 1,
                        const size_t maxSampleSize = 64225280, const std::string gpatch_path = "",
                        const bool directHostAccess = false, const int deviceId = 0,
                        const int NUMAidx = -1, const int numNUMA = 1)
        : mDirectHostAccess(directHostAccess), mMaxSampleVol(maxSampleSize), mDeviceId(deviceId),
          mNUMAidx(NUMAidx), mNumNUMA(numNUMA)
    {
        ...
        // Each optimization profile owns numBindings bindings.
        // Create host and device buffers
        // SW: input is 32ch INT8, output is 3ch FP16
        size_t sw_vol = SW_dhw * SW_dhw * SW_dhw;
        size_t sw_input_alloc = sw_vol * /*int8*/ 1 * /*32ch*/ 32;
        ...
        m_SW_gaussian_patch_size =
            m_SW_gaussian_patches_npy->getTensorSize() / m_SW_gaussian_patch_num;
        ...
        for(int i = 0; i < m_SW_gaussian_patch_num; ++i)
        {
            if(!mDirectHostAccess)
            {
                ...
                CHECK_CUDA(cudaMemcpy(mManagedBuffers->gaussianPatchesDevice[i].data(),
                                      gpatch_data.data() + i * m_SW_gaussian_patch_size,
                                      m_SW_gaussian_patch_size, cudaMemcpyHostToDevice));
            }
            else
            {
    ...
    void setupBuffers(const int sample_d, const int sample_h, const int sample_w,
                      const cudaStream_t& stream = 0, const bool async = false)
    {
        ...
        // optimized, max size buffer usage -- FIXME: accuracy drops
        if(!mDirectHostAccess)
        {
            if(async)
            {
                CHECK_CUDA(cudaMemsetAsync(getSampleInputDeviceBuffer(), 0, mMaxSampleVol * 1 * 1,
                                           stream));
                CHECK_CUDA(cudaMemsetAsync(getSampleOutputDeviceBuffer(), 0, mMaxSampleVol * 2 * 3,
                                           stream));
                CHECK_CUDA(cudaMemsetAsync(getSampleResponseDeviceBuffer(), 0,
                                           mMaxSampleVol * 1 * 1, stream));
            }
            else
            {
    ...
    {
        return mDirectHostAccess ? getGaussianPatchHostBuffer(index)
                                 : getGaussianPatchDeviceBuffer(index);
    }

    // copy the contents of input host buffers to input device buffers asynchronously
    void copyInputToDeviceAsync(const cudaStream_t& stream = 0, void* src = nullptr,
                                const size_t size = 0)
    {
  ...
  private:
    ...
    //                     D2H
    void memcpyBuffers(
        const bool H2D, // true: case 1), false: case 2)
    ...
    {
        const void* srcPtr =
            H2D ? (buf ? buf : getSampleInputHostBuffer()) : getSampleResponseDeviceBuffer();
        void* dstPtr = H2D ? getSampleInputDeviceBuffer() : getSampleResponseHostBuffer();
        const size_t byteSize =
            H2D ? (buf && (size > 0) ? size : getSampleInputSize()) : getSampleResponseSize();
