...
class Triton_Server_SUT : public mlperf::SystemUnderTest
{
  public:
    ...
    Triton_Server_SUT(const std::string& name, const std::string& model_repo_path,
                      const std::string& model_name, uint32_t model_version, bool use_dlrm_qsl,
                      bool start_from_device, bool end_on_device, bool pinned_input,
        ...
        : m_Name(name), m_ModelRepositoryPath(model_repo_path), m_ModelName(model_name),
          ...
          m_StartFromDevice(start_from_device), m_EndOnDevice(end_on_device),
    ...
    virtual void
        ReportLatencyResults(const std::vector<mlperf::QuerySampleLatency>& latencies_ns) = 0;
    ...
    virtual void Completion(TRITONSERVER_InferenceResponse* response,
                            const ResponseMetaData* response_metadata) = 0;
    virtual void BatchCompletion(TRITONSERVER_InferenceResponse* response,
                                 const BatchResponseMetaData* response_metadata) = 0;
    ...
    virtual void
        SetResponseCallback(std::function<void(::mlperf::QuerySampleResponse* responses,
    ...
    virtual void QuerySamplesComplete(mlperf::QuerySampleResponse* responses, size_t num_responses,
                                      int64_t pool_idx);
