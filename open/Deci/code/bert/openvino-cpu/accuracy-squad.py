...
from squad_utils.utils_squad import postprocess_qa_predictions
...
def main():
    ...
    id_order = [x.unique_id for x in eval_features]
    results_id_order = [x.unique_id for x in results]
    results_order = []
    for id in id_order:
        if id in results_id_order:
            results_order.append(results[results_id_order.index(id)])
    start_ind = [x[1] for x in results_order]
    end_ind = [x[2] for x in results_order]
    results_for_post_processing = (np.asarray(start_ind), np.asarray(end_ind))

    predictions = postprocess_qa_predictions(
        examples=eval_examples,
        features=eval_features,
        predictions=tuple(results_for_post_processing),
        version_2_with_negative=False,
        n_best_size=20,
        max_answer_length=30,
        null_score_diff_threshold=0,
        output_dir="/tmp/",
        output_prediction_file=args.out_file
    )
    ...
    cmd = "python3 {:}/evaluate-v1.1.py {:} {:} {}".format(
        ...
        args.out_file, '--max_examples {}'.format(
            args.max_examples) if args.max_examples else '') + " --out_file " \
          + os.path.join(os.path.dirname(args.out_file), "accuracy.txt")
