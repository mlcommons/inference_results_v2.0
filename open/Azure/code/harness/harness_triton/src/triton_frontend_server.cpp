...
void Triton_Server_SUT::Init(size_t min_sample_size, size_t max_sample_size,
                             ...
                             const std::string& backend_path)
{
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetPinnedMemoryPoolByteSize(server_options,
                                                                      (((uint64_t)1) << 29)),
    ...
    for(size_t node_idx = 0; node_idx < numa_config.size(); ++node_idx)
    {
        ...
        for(const auto gpu_id : gpu_ids)
        {
            ...
            FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetHostPolicy(server_options, policy_name.c_str(),
                                                                "numa-node",
                                                                std::to_string(node_idx).c_str()),
                        "setting NUMA node for host policy");
            FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetHostPolicy(server_options, policy_name.c_str(),
                                                                "cpu-cores", cpu_str.c_str()),
...
void Triton_Server_SUT::TraceCaptureTimeStamp(TRITONSERVER_InferenceTrace* trace_ptr,
...
void Triton_Server_SUT::QuerySamplesComplete(mlperf::QuerySampleResponse* responses,
                                             size_t num_responses, int64_t pool_idx)
{
    if(m_EndOnDevice)
    {
        ...
        const auto end_on_device_cb = [=](::mlperf::QuerySampleResponse* response) {
            ...
            FAIL_IF_CUDA_ERR(cudaMemcpyAsync(host_buf, (const void*)response->data, response->size,
