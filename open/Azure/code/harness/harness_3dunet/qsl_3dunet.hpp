...
struct kits19_dim
{
    ...
    kits19_dim(int _c, int _d, int _h, int _w) : c(_c), d(_d), h(_h), w(_w) {}
};
...
class LookupableQuerySampleLibrary : public mlperf::QuerySampleLibrary
{
  public:
    ...
    virtual kits19_dim GetSampleDimension(mlperf::QuerySampleIndex sample_index,
                                          size_t input_idx) const = 0;
};
  ...
  public:
    SampleLibrary3DUNet(std::string name, std::string mapPath, std::vector<std::string> tensorPaths,
        ...
        : m_Name(name), m_PerfSampleCount(perfSampleCount), m_PerfSamplePadding(padding),
          m_Coalesced(coalesced), m_MapPath(mapPath), m_TensorPaths(tensorPaths)
    {
        ...
        for(size_t input_idx = 0; input_idx < m_NumInputs; input_idx++)
        {
            ...
            for(size_t sample_idx = 0; sample_idx < m_SampleCount; sample_idx++)
            {
                std::string path = m_TensorPaths[input_idx] + "/" +
                                   std::get<0>(m_FileLabelMap[sample_idx]) + ".npy";
                ...
                CHECK(sample_dim.size() == 4)
                    << "Expected 4D tensor, but got " << sample_dim.size()
            ...
            else
            {
                ...
                for(int device_idx = 0; device_idx < m_NumDevices; device_idx++)
                {
                    ...
                    CHECK_EQ(cudaMalloc(&m_SampleMemory[input_idx][device_idx], sampleSizeSum),
                             cudaSuccess);
                }
            }
        }
    ...
    void LoadSamplesToRam(const std::vector<mlperf::QuerySampleIndex>& samples) override
    {
        ...
        for(size_t input_idx = 0; input_idx < m_NumInputs; input_idx++)
        {
            ...
            for(size_t sampleIndex = 0; sampleIndex < samples.size(); sampleIndex++)
            {
                ...
                std::string path =
                    m_TensorPaths[input_idx] + "/" + std::get<0>(m_FileLabelMap[sampleId]) + ".npy";
                npy::NpyFile npy(path);
                ...
                {
                    auto sampleAddress =
                        static_cast<int8_t*>(m_SampleMemory[input_idx][0]) + addressOffset;
                    std::memcpy((char*)sampleAddress, data.data(),
                                m_SampleSizes[sampleId][input_idx]);
                ...
                else
                {
                    for(int device_idx = 0; device_idx < m_NumDevices; device_idx++)
                    {
                        auto sampleAddress =
                            ...
                            addressOffset;
                        CHECK_EQ(cudaSetDevice(device_idx), cudaSuccess);
                        CHECK_EQ(cudaMemcpy(sampleAddress, data.data(),
                                            m_SampleSizes[sampleId][input_idx],
        ...
        if(std::any_of(m_StartFromDevice.begin(), m_StartFromDevice.end(),
    ...
    void* GetSampleAddress(mlperf::QuerySampleIndex sample_idx, size_t input_idx,
                           size_t device_idx = 0) override
    {
        if(!m_StartFromDevice[input_idx])
        {
            ...
            CHECK(it != m_SampleAddressMapHost.end())
                << "Sample: " << sample_idx << " missing from RAM";
        ...
        CHECK(it != m_SampleAddressMapDevice.end())
            << "Sample: " << sample_idx << " missing from Device RAM";
        CHECK(input_idx <= it->second.front().size()) << "invalid input_idx";
    ...
    kits19_dim GetSampleDimension(mlperf::QuerySampleIndex sample_idx,
                                  size_t input_idx) const override
    {
        return (m_SampleDimensions.empty() ? kits19_dim{0, 0, 0, 0}
                                           : m_SampleDimensions[sample_idx][input_idx]);
    }
...
{
  public:
    SampleLibrary3DUNetEnsemble(const std::vector<SampleLibrary3DUNetPtr_t> qsls) : m_qsls(qsls){};
    const std::string& Name() const override
    {
