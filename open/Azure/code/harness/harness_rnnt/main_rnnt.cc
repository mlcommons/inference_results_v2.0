...
DEFINE_string(logfile_outdir, "build/logs",
              "Specify the existing output directory for the LoadGen logs");
...
int main(int argc, char* argv[])
{
    ...
    // Scope to force all smart objects destruction before CUDA context resets
    {
        ...
        auto num_warmup_samples =
            FLAGS_num_warmups == -1 ? default_num_warmup_samples : FLAGS_num_warmups;
        auto wqsl = std::make_shared<WarmupSampleLibrary>(num_warmup_samples,
                                                          FLAGS_enable_audio_processing);
